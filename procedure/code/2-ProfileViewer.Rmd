...
CrossProf <- read.csv("../../data/derived/public/cross_prof_gsm.txt", header=FALSE, sep = " ", col.names=c("Junk","Distance","Elevation"))[c(2,3)]
LongProf <- read.csv("../../data/derived/public/long_prof_gsm.txt", header=FALSE, sep = " ", col.names=c("Junk","Distance","Elevation"))[c(2,3)]
```
...
ggplot(LongProf) + # This sets the variable (data frame) we will be plotting.
       ...
       labs(x = "Distance along the transect (m)", y = "Elevation (masl)", title = "Longitudinal Profile of Vinegar Creek, Reach 17")
...
ggplot(CrossProf) +
       ...
       labs(x = "Distance along the transect (m)", y = "Elevation (masl)", title = "Cross-Sectional Profile of Valley Near Vinegar Creek")
```
...
ggplot(LongProf) +
       ...
       labs(x = "Distance along the transect (m)", y = "Slope (%)", title = "Slope of Vinegar Creek, Reach 17")
...
# ***NOTE: this method of calculating slope exacerbates errors in digitizing stream banks and appears to be inaccurate in the context of this analysis. The final classification uses a simple calculation of rise/run along the reach to calculate slope.

# Now we want to figure out the width of the flood-prone area.
DpthBf_Max <- 0.7876 # Insert the maximum bankfull depth from your point/reach of interest here, replacing the 8 left of the hash #
DpthBf_Avg <- 0.2428 # Avg bankfull depth used for width/depth ratio
...
MaxBFElev <- StreamElev + DpthBf_Avg
...
ggplot(CrossProf) +
       ...
       geom_hline(yintercept = MaxBFElev, color = "blue") +
       xlim(210,290) + # Sets the x-axis limits. Adjust this value as you zoom in to the correct area.
       ylim(1234,1242) + # Sets the y-axis limits. Adjust this value as you zoom in to the correct area.
       scale_color_gradient(low = "green", high = "brown") +
       labs(x = "Distance along the transect (m)", y = "Elevation (masl)", title = "Flood-Prone Width of Vinegar Creek, Reach 17")
